

import sys
import numpy as np
import matplotlib.pyplot as plt
from scipy.optimize import curve_fit


# ==============================================================================
# THERMISCHE MATERIALPARAMETER
# ==============================================================================

# Substrat: Armco-Reineisen
K_SUBSTRAT = 73.0       # Wärmeleitfähigkeit Substrat [W/(m·K)]
RHO_SUBSTRAT = 7870.0   # Dichte Substrat [kg/m³]
C_SUBSTRAT = 460.0      # Spezifische Wärmekapazität Substrat [J/(kg·K)]

# Nitrierschicht (Verbindungsschicht)
RHO_LAYER = 7870.0      # Dichte Schicht [kg/m³]
C_LAYER = 460.0         # Spezifische Wärmekapazität Schicht [J/(kg·K)]
K_LAYER_DEFAULT = 14.4  # Standard-Wärmeleitfähigkeit Schicht [W/(m·K)]


# ==============================================================================
# HELFER & MATH. PHYSIK-MODELL (Formel 2 nach Mikulewitsch et al.)
# ==============================================================================

def get_effusivity(k, rho, c):
    """ Berechnet die thermische Effusivität b = sqrt(k * rho * c) """
    return np.sqrt(k * rho * c)


def get_diffusivity(k, rho, c):
    """ Berechnet die thermische Diffusivität alpha = k / (rho * c) in m²/s """
    return k / (rho * c)


def fit_model_d_only(f, d_um, k_L=K_LAYER_DEFAULT):
    """
    Berechnet das theoretische Phasensignal φ(f, d) nach Formel 2.
    
    Parameters:
        f    : Modulationsfrequenz [Hz] (Numpy-Array oder Float)
        d_um : Schichtdicke d in µm
        k_L  : Wärmeleitfähigkeit der Schicht [W/(m·K)]
        
    Returns:
        Phase in Grad (°)
    """
    omega = 2.0 * np.pi * f
    d = d_um * 1e-6  # Umrechnung µm -> m

    # Effusivitäten & Diffusivität
    b_L = get_effusivity(k_L, RHO_LAYER, C_LAYER)
    b_S = get_effusivity(K_SUBSTRAT, RHO_SUBSTRAT, C_SUBSTRAT)
    alpha_L = get_diffusivity(k_L, RHO_LAYER, C_LAYER)

    # Thermische Diffusionslänge μ_L
    mu_L = np.sqrt(2.0 * alpha_L / omega)
    term_d_mu = (2.0 * d) / mu_L

    b_diff = b_S - b_L
    b_sum  = b_S + b_L

    # Zähler und Nenner von Formel 2
    numerator = (b_S**2 - b_L**2) * (np.exp(term_d_mu) - np.exp(-term_d_mu)) + 2.0 * (b_S**2 - b_L**2) * np.sin(term_d_mu)
    denominator = -(b_diff**2) + (b_sum**2) * np.exp(2.0 * term_d_mu) + 2.0 * (b_S**2 - b_L**2) * np.sin(term_d_mu)

    phi_rad = np.arctan(numerator / denominator)
    return np.degrees(phi_rad)


# ==============================================================================
# EIGENTLICHE EVALUATIONS-FUNKTION FÜR DIE GUI
# ==============================================================================

def process_live_measurement(f_measured, phi_measured, initial_d_guess=10.0):
    """
    Verarbeitet Messdaten vom Lock-In Verstärker und führt Non-Linear Fitting aus.
    
    Parameters:
        f_measured   : 1D-Numpy-Array der Modulationsfrequenzen [Hz]
        phi_measured : 1D-Numpy-Array der gemessenen Phasenwerte [°]
        initial_d_guess : Startwert für die Schichtdicke in µm
        
    Returns:
        d_fitted_um     : Gefittete Schichtdicke d [µm]
        d_error_um      : Standardabweichung / Fit-Unschärfe [µm]
        r_squared       : Bestimmtheitsmaß R² zur Qualitätskontrolle (0.0 bis 1.0)
        phi_model_fitted: Gefittetes Signal als Numpy-Array (gleiche Länge wie f_measured)
    """
    # 1. Non-linear Curve Fitting (scipy.optimize.curve_fit)
    popt, pcov = curve_fit(
        fit_model_d_only, 
        f_measured, 
        phi_measured, 
        p0=[initial_d_guess], 
        bounds=(0.1, 500.0)  # Schichtdicke physikalisch eingrenzen (0.1 bis 500 µm)
    )

    d_fitted_um = popt[0]
    d_error_um = np.sqrt(np.diag(pcov))[0]

    # 2. Verifikation & Qualitätskontrolle (R²-Berechnung)
    phi_model_fitted = fit_model_d_only(f_measured, d_fitted_um)
    residuals = phi_measured - phi_model_fitted
    ss_res = np.sum(residuals**2)
    ss_tot = np.sum((phi_measured - np.mean(phi_measured))**2)
    r_squared = 1.0 - (ss_res / ss_tot) if ss_tot != 0 else 0.0

    # 3. Konsolen-Ausgabe
    print("=" * 60)
    print("  PTR MESSDATEN-EVALUATION & VERIFIKATION")
    print("=" * 60)
    print(f"  Gemessene Punkte:           {len(f_measured)}")
    print(f"  Frequenzbereich:            {f_measured.min():.1f} Hz bis {f_measured.max():.1f} Hz")
    print("-" * 60)
    print(f"  --> GEFITTETE SCHICHTDICKE d: {d_fitted_um:.2f} ± {d_error_um:.2f} µm")
    print(f"  --> BESTIMMTHEITSMASS (R²):  {r_squared:.4f}")
    
    if r_squared >= 0.95:
        print("  --> STATUS:                  FIT VALIDE UND PRÄZISE")
    else:
        print("  --> WARNUNG:                 R² NIEDRIG! MESSDATEN ODER RAUSCHEN PRÜFEN.")
    print("=" * 60)

    return d_fitted_um, d_error_um, r_squared, phi_model_fitted


# ==============================================================================
# DEMO / DIRECT EXECUTION (Simulation mit Testwert zur Überprüfung, ob das Ding überhaupt läuft)
# ==============================================================================

#if __name__ == "__main__":
    print("Starte Demo-Test mit synthetischen PTR-Messdaten...\n")
    
    # 1. Messdaten des Lock-In-Verstärkers simulieren (z. B. 10 Hz bis 250 kHz)
    np.random.seed(42)
    # Sweep-Bereich: 10^0 = 1 Hz bis 10^5 = 100.000 Hz mit 500 Punkten
    f_simulated = np.logspace(0, 5, 250)
    
    # Reale Schichtdicke von d in µm simulieren
    d_true = 15.0
    noise = np.random.normal(0, 0.35, size=len(f_simulated))  # Experimentelles Rauschen
    phi_simulated = fit_model_d_only(f_simulated, d_true) + noise

    # 2. Auswertungsfunktion aufrufen
    d_fit, d_err, r2, phi_fit = process_live_measurement(f_simulated, phi_simulated)

    # 3. Ergebnis in Matplotlib darstellen
    plt.figure(figsize=(9, 5))
    
    sqrt_omega_sim = np.sqrt(2 * np.pi * f_simulated)
    f_dense = np.logspace(1, 5.4, 500)
    sqrt_omega_dense = np.sqrt(2 * np.pi * f_dense)
    phi_dense_fit = fit_model_d_only(f_dense, d_fit)

    plt.plot(sqrt_omega_sim, phi_simulated, 'ro', alpha=0.7, label='Messdaten (Lock-In)')
    plt.plot(sqrt_omega_dense, phi_dense_fit, 'b-', linewidth=2, label=f'Fit (d = {d_fit:.2f} µm, R² = {r2:.4f})')
    
    plt.xlabel(r'$\sqrt{\omega}$ [$\sqrt{\mathrm{Hz}}$]', fontsize=11)
    plt.ylabel('Phase φ [°]', fontsize=11)
    plt.title('Photothermische Radiometrie - Nitrierschichtdicken-Fit', fontsize=12, fontweight='bold')
    plt.grid(True, linestyle=':', alpha=0.7)
    plt.legend(fontsize=10)
    plt.tight_layout()
    plt.show()